...
def Co_express_network(gene_express, prior_marker,target_size=350,output_dir=None):
    ...
    for idx in cell_type_indices:
        ...
        specified_genes = [gene for gene in specified_genes if gene in gene_express.columns]
        corr_threshold=0.6
        ...
        while True:
            for gene in specified_genes:
                if gene not in correlation_matrix.columns:
                    continue

                # Extract correlations and p-values for the given gene
                correlations = correlation_matrix[gene]
                p_values = p_value_matrix[gene]
...
def Co_regulatory_network(coexpression_networks, GRN,output_file: Optional[str] = None):
    edges = [(u, v, data['weights']) for u, v, data in GRN.edges(data=True)]
    GRN = pd.DataFrame(edges, columns=['from', 'to', 'weights'])
    ...
    for cell_type in coexpression_networks.keys():
        ...
        specified_genes_df = pd.DataFrame(specified_genes_df, columns=['gene'])
        ...
        predEdgesDF2 = GRN[GRN['Gene2'].isin(filtered_df['Gene2'])]
        ...
        Candidate_marker_out_number = top_50_percent_df['Candidate_marker'].value_counts().reset_index()
